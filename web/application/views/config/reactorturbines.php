    ...
    <ul id="turbine-list">
        <?php for($i=0; $i<count($turbines); $i++) : ?>
            ...
            <li class="clearfix">
                    ...
                    <div class="config-container clearfix" id="turbine_config_<?php echo($i); ?>">
                        <?php for($j=0; $j<count($turbine->configurationProperties); $j++) : ?>
                            ...
                            <div class="config-property-container clearfix">
                                ...
                                <input name="<?php echo($config->name); ?>" type="text" onchange="SaveConfiguration('<?php echo($turbine->name); ?>', '<?php echo($i); ?>', 'reactor')" class="config-property-value <?php echo($config->type); ?>" id="config_<?php echo($i); ?>_<?php echo($j); ?>" value="<?php echo($config->value); ?>"/>
                ...
                <?php endif; ?>
                <p class="active" style="<?php if(!$turbine->active) echo("display:none"); ?>" id="active_<?php echo($i); ?>"><?php echo(Html::image("media/images/button-activate.png", array())); ?>Currently active, <a href="javascript:DeactivateTurbine(<?php echo($i); ?>, '<?php echo($turbine->name); ?>', 'reactor')">deactivate?</a></p>
                <p class="active" style="<?php if($turbine->active) echo("display:none"); ?>" id="inactive_<?php echo($i); ?>"><?php echo(Html::image("media/images/button-deactivate.png", array())); ?>Not currently active, <a href="javascript:ActivateTurbine(<?php echo($i); ?>, '<?php echo($turbine->name); ?>', 'reactor')">actiavte?</a></p>
            </li>
        <?php endfor; ?>
    </ul>
</div>